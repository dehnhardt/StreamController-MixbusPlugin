...
class Undo(MixbusActionBase):
    def __init__(self, *args, **kwargs):
        log.debug( "__init__")
        super().__init__(*args, **kwargs)
        self.current_state = -1
        #We have no feedback here..
        # self.plugin_base.connect_to_event(event_id="org_dehnhardt_MixbusPlugin::Undo",
    ...
    def on_key_down(self) -> None:
        try:
            self.plugin_base.backend.send_message("/undo", 1 )         
    ...
    #         return
    #     state = args[2]
    #     log.debug( "on undo - status " + str( state ))
